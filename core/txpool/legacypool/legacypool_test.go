...
func (bc *testBlockChain) GetBlock(hash common.Hash, number uint64) *types.Block {
	return types.NewBlock(bc.CurrentHeader(), nil, nil, nil, trie.NewStackTrie(nil))
}
...
func setupPoolWithConfig(config *params.ChainConfig) (*LegacyPool, *ecdsa.PrivateKey) {
	...
	if err := pool.Init(new(big.Int).SetUint64(testTxPoolConfig.PriceLimit), blockchain.CurrentHeader(), makeAddressReserver()); err != nil {
		panic(err)
	}
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	pool := New(testTxPoolConfig, blockchain)
	pool.Init(new(big.Int).SetUint64(testTxPoolConfig.PriceLimit), blockchain.CurrentHeader(), makeAddressReserver())
	defer pool.Close()
...
func testQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	pool := New(config, blockchain)
	pool.Init(new(big.Int).SetUint64(config.PriceLimit), blockchain.CurrentHeader(), makeAddressReserver())
	defer pool.Close()
